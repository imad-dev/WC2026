...
export function UpcomingFixtures({ fixtures: apiFixtures, loading }: UpcomingFixturesProps) {
  // Use API data or mock data
  const upcomingFixtures = apiFixtures && apiFixtures.length > 0
    ? apiFixtures.filter((f: any) => f.status === 'UPCOMING').slice(0, 5)
    : [];

  const fixtures = upcomingFixtures.length > 0 ? upcomingFixtures : [
